...
image, port and command to build a suitable task definition. Set `service_use_latest_task_definition = true` to use the
...
| service_deployment_circuit_breaker_rollback | Whether the circuit breaker rolls back to the last healthy deployment on failure |           false           |                                   no                                    |
